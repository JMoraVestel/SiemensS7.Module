...
{
    /// <summary>
    /// Diagnostic metrics for a channel and all its devices and tags
    /// </summary>
    public class ChannelDiagnostics : DiagnosticMetrics, IDisposable
    {
        ...
        public ChannelDiagnostics(Dictionary<string, ModbusDeviceConfig> devices, ISdkLogger logger, int rollingWindowSize = 10)
        {
            _logger = logger;
            ...
            foreach (var device in devices)
            {
                ...
                var deviceDiagnostics = new DeviceDiagnostics(device.Key, $"Device-{deviceId}", _logger, _rollingWindowSize);
        ...
        public void UnregisterTag(Guid tagId)
        {
            ...
            {
                _logger.Warning("ChannelDiagnostics", $"Cannot unregister tag id [{tagId}] because is not registered");
            }
        }
